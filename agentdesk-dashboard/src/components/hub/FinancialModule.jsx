...
export default function FinancialModule({ agentId }) {
  ...
  const cargarHistorial = useCallback(async () => {
    ...
    try {
      ...
      /* historial puede estar vacío en primer uso */
    }
  }, [agentId]);
  ...
  const ejecutarAnalisis = async () => {
    if (!agentId) {
      setError("Selecciona un agente primero.");
      return;
    }
    const itemsInvalidos = items.filter(
      (i) => !i.concepto.trim() || i.monto < 0,
    );
    if (itemsInvalidos.length) {
      setError("Revisa los ítems: concepto vacío o monto negativo.");
      return;
    }
          ...
          {items.map((item, idx) => (
            <div key={idx} className="flex items-center gap-2">
              ...
              <input
                ...
                value={item.concepto}
                onChange={(e) =>
                  actualizarItem(idx, "concepto", e.target.value)
                }
              ...
              <input
                ...
                value={item.monto}
                onChange={(e) =>
                  actualizarItem(idx, "monto", Number(e.target.value))
                }
      ...
      {flujoActual && (
        <section className="grid grid-cols-3 gap-3">
          {[
            {
              label: "Ingresos",
              value: flujoActual.ingresos,
              color: "text-neon-green",
            },
            {
              label: "Egresos",
              value: flujoActual.egresos,
              color: "text-red-400",
            },
            {
              label: "Flujo Neto",
              value: flujoActual.flujo_neto,
              color:
                flujoActual.flujo_neto >= 0 ? "text-neon-blue" : "text-red-400",
            },
          ].map(({ label, value, color }) => (
            <div
              key={label}
              className="bg-cyber-800 rounded-xl border border-neon-blue/10 p-3 text-center"
            >
              ...
              <p className="text-xs text-gray-600">
                {flujoActual.moneda} / {flujoActual.periodo}
              </p>
            </div>
          ))}
        </section>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {historial.map((r, i) => (
                  <tr
                    key={r.id ?? i}
                    className="border-b border-cyber-700/50 hover:bg-cyber-700/30 transition-colors"
                  >
                    ...
                    <td className="py-1.5 text-right font-mono text-gray-300">
                      {r.dolar_valor != null
                        ? `$${fmtNum(r.dolar_valor)}`
                        : "—"}
                    </td>
                    <td
                      className={`py-1.5 text-right font-mono font-semibold ${(r.flujo_neto ?? 0) >= 0 ? "text-neon-green" : "text-red-400"}`}
                    >

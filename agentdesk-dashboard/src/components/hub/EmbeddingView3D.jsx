...
export default function EmbeddingView3D() {
  ...
  const load = useCallback(() => {
    ...
    tryEmb
      ...
      .then((pts) => {
        ...
        setAreas([
          "Todos",
          ...new Set(pts.filter((p) => p.tipo === "agente").map((p) => p.area)),
        ]);
        setLoading(false);
      })
  ...
  useEffect(() => {
    ...
    function frame() {
      ...
      // Wireframe cubo
      ctx.strokeStyle = "rgba(22,36,84,.7)";
      ctx.lineWidth = 0.8;
      ...
      const projected = visible
        .map((p) => ({
          ...p,
          ...project3D(p.x, p.y, p.z, st.rx, st.ry, cx, cy, fov, scale),
        }))
        .sort((a, b) => b.z - a.z);
      ...
      for (const p of projected) {
        ...
        // Punto
        ctx.beginPath();
        ctx.arc(p.px, p.py, Math.max(r, 1.2), 0, Math.PI * 2);
        ...
        if (p.tipo === "agente" && r > 2) {
          ...
          ctx.font = `bold ${Math.max(10, r * 1.8)}px monospace`;
          ctx.textAlign = "center";
      ...
      if (!selId) {
        ...
        if (hp) {
          ...
          ctx.lineWidth = 1.2;
          ctx.beginPath();
          ctx.roundRect(tx, ty, tw, lines.length * lh + pad, 6);
          ctx.fill();
          ctx.stroke();
          lines.forEach((l, i) => {
            ctx.fillStyle = i === 0 ? hp.color : "#94a3b8";
            ctx.font = `${i === 0 ? "bold 12" : "11"}px monospace`;
            ctx.textAlign = "left";
      ...
      ctx.textAlign = "left";
      ctx.fillText(
        `Rx:${(st.rx * 57.3).toFixed(0)}°  Ry:${(st.ry * 57.3).toFixed(0)}°  Zoom:${st.zoom.toFixed(2)}x`,
        10,
        H - 10,
      );
      ...
      ctx.fillText(
        "Arrastra · Scroll zoom · Click agente · Doble clic = reset",
        W - 10,
        H - 10,
      );
    ...
    function onTouchMove(e) {
      ...
      if (e.touches.length === 1 && st.drag) {
        const dx = e.touches[0].clientX - st.lx,
          dy = e.touches[0].clientY - st.ly;
        st.ry += dx * 0.007;
        st.rx += dy * 0.007;
        st.lx = e.touches[0].clientX;
        st.ly = e.touches[0].clientY;
    ...
    function onTouchEnd(e) {
      ...
      if (e.touches.length === 0) {
        if (st.drag && st.dragDist < 10) {
          ...
          const rect = canvas.getBoundingClientRect();
          const tx =
            ((e.changedTouches[0].clientX - rect.left) * canvas.width) /
            rect.width;
          const ty =
            ((e.changedTouches[0].clientY - rect.top) * canvas.height) /
            rect.height;
  ...
  return (
    <div
      ...
      style={{
        ...
        gap: ".8rem",
        height: isFullscreen ? "100vh" : "calc(100vh - 175px)",
        minHeight: 480,
    ...
    >
      ...
      >
        ...
        >
          ...
          <button
            onClick={resetVista}
            title="Reset vista (doble clic en canvas)"
            style={{
              padding: "3px 9px",
              borderRadius: 20,
              fontSize: ".75rem",
          ...
          >
            <Maximize2 size={12} /> {isFullscreen ? "Salir" : "Ampliar"}
          </button>
          ...
          >
            ...
            >
              {[
                [
                  "Modelo",
                  (selected.modelo || "gemini-2.5-flash").split("/").pop(),
                ],
                ...
                [
                  "Satélites",
                  String(
                    puntosRef.current.filter((p) => p.parentId === selected.id)
                      .length,
                  ),
                ],
              ].map(([k, v]) => (
      ...
      >
        {Object.entries(AREA_COLORS).map(([a, c]) => (
          <button
            key={a}
            onClick={() => setFilter((f) => (f === a ? "Todos" : a))}
            style={{
              display: "flex",
              alignItems: "center",
              gap: 4,
        ...
        <span
          style={{ marginLeft: "auto", color: "#334155", fontSize: ".65rem" }}
        >
          🖱 Arrastra·rotar | Scroll·zoom | Click·seleccionar | Doble clic·reset
          | Toca leyenda·filtrar
        </span>
      </div>
    </div>
  );

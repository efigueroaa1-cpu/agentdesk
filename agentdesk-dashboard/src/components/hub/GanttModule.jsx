    ...
    >
      ...
      {wDone > 0 && (
        <rect
          ...
          height={BAR_H}
          rx={4}
          fill={critica ? CRITICO_CLR : color}
          opacity={0.85}
        />
      )}
      {/* Etiqueta */}
...
function NuevaTareaModal({ proyectoId, agentes, tareas, onCreada, onClose }) {
  ...
  const submit = async () => {
    ...
    if (form.duracion_dias <= 0) {
      setErr("La duración debe ser positiva.");
      return;
    }
    setLoading(true);
    setErr("");
      ...
      <div className="bg-cyber-800 border border-neon-blue/30 rounded-2xl p-6 w-[440px] max-w-full shadow-2xl">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label
                htmlFor="gantt-inicio-plan"
                className="text-xs text-gray-500 mb-1 block"
              >
                Inicio planificado
              </label>
              <input
                id="gantt-inicio-plan"
                type="date"
                value={form.inicio_plan}
                onChange={(e) => set("inicio_plan", e.target.value)}
                className="w-full bg-cyber-700 border border-cyber-600 rounded text-sm text-white px-2 py-1.5"
              />
            </div>
            <div>
              <label
                htmlFor="gantt-duracion"
                className="text-xs text-gray-500 mb-1 block"
              >
                Duración (días)
              </label>
          ...
          <div className="flex items-center gap-2">
            <label htmlFor="gantt-color" className="text-xs text-gray-500">
              Color
            </label>
            <input
              id="gantt-color"
...
export default function GanttModule({
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `avance_${proyectoId}.pdf`;
      a.click();
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-gray-500">
            Proyecto: <span className="text-neon-blue">{proyectoId}</span>
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
          ...
          <button
            ...
            disabled={exporting || tareas.length === 0}
      ...
      {Object.keys(resumen).length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {[
            {
              label: "Avance global",
              value: `${resumen.pct_avance ?? 0}%`,
              icon: CheckCircle2,
              color: "text-neon-green",
            },
            {
              label: "Total tareas",
              value: resumen.total_tareas ?? 0,
              icon: Clock,
              color: "text-neon-blue",
            },
            {
              label: "Ruta crítica",
              value: `${resumen.tareas_criticas ?? 0} tareas`,
              icon: AlertTriangle,
              color: "text-red-400",
            },
            {
              label: "Fin planificado",
              value: (resumen.fecha_fin || "—").slice(0, 10),
              icon: Clock,
            ...
            <div
              key={label}
              className="bg-cyber-800 border border-neon-blue/10 rounded-xl p-3 flex items-center gap-2"
            >
      ...
      {/* Loading */}
      {loading && (
        <div className="text-xs text-gray-500 animate-pulse py-8 text-center">
          Cargando cronograma…
        </div>
      )}
          ...
          <div className="overflow-x-auto" style={{ maxWidth: "100%" }}>
            ...
            >
              ...
              {tareas.map((t) => {
                ...
                const w = Math.max(
                  6,
                  daysBetween(parseDate(t.inicio_plan), parseDate(t.fin_plan)) *
                    PX_DAY,
                );
                const y = yOf(t.id);
                return (
          ...
          <div className="flex items-center gap-5 px-4 py-2.5 border-t border-neon-blue/10 text-xs text-gray-500">
            <span className="flex items-center gap-1.5">
              <span
                ...
                style={{ background: AVANCE_CLR }}
              />{" "}
              Avance real
            </span>
            <span className="flex items-center gap-1.5">
              <span
                className="w-4 h-2.5 rounded border border-red-500"
            ...
            <span className="ml-auto text-gray-600">
              Escala: {PX_DAY}px/día · Clic en tarea para seleccionar
            </span>
          </div>
        </div>
      )}

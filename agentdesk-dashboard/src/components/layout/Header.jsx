...
export default function Header({
  ...
  return (
    <header className="h-16 bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 flex items-center gap-4 px-5 shrink-0 transition-colors">
      <button
        onClick={onMenuClick}
        className="lg:hidden text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white shrink-0"
      >
        <Menu size={22} />
      </button>
      <h1 className="text-lg font-semibold text-gray-800 dark:text-white hidden sm:block shrink-0">
        {title}
      </h1>
      <div className="hidden sm:block w-px h-5 bg-gray-200 dark:bg-gray-600 shrink-0" />
      <div className="flex-1 max-w-md">
        <div
          className={`relative flex items-center rounded-xl overflow-hidden transition-all ${focused ? "ring-2 ring-blue-400 bg-white dark:bg-gray-700" : "bg-gray-100 dark:bg-gray-700"}`}
        >
          <Search
            size={15}
      ...
      <div className="flex items-center gap-2 ml-auto shrink-0">
        {onToggleDark && (
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded-xl text-gray-500 dark:text-yellow-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
          >

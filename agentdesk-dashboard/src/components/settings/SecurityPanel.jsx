 ...
 *   3. Kill Switch por licencia RSA local (activa/desactiva ejecución de agentes)
...
function rolColor(rol) {
  return (
    { admin: "#00d4ff", supervisor: "#a78bfa", viewer: "#64748b" }[rol] ??
    "#64748b"
  );
}
      ...
      {hash && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <p style={{ fontSize: 11, color: "var(--t-text-muted)", margin: 0 }}>
            Pega este hash en{" "}
            <code style={{ color: "var(--t-accent)" }}>.env</code> como{" "}
            <code style={{ color: "var(--t-accent)" }}>
...
function UserManager({ token }) {
  ...
  const cargarUsuarios = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 403) {
        showMsg("Se requiere rol admin.", "error");
        return;
      }
  ...
  const handleCreate = async () => {
    if (!newUser.username.trim() || !newUser.password.trim()) {
      showMsg("Usuario y contraseña son obligatorios.", "error");
      return;
    }
    setSaving(true);
    setMsg(null);
  ...
  const handleDelete = async (username) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/auth/usuarios/${encodeURIComponent(username)}`,
        {
          method: "DELETE",
          headers: authHeaders,
        },
  ...
  const handleCambiarRol = async (username, nuevo_rol) => {
    try {
      const res = await fetch(
        `${API_BASE}/auth/usuarios/${encodeURIComponent(username)}/rol`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json", ...authHeaders },
  ...
  const handleToggleActivo = async (username, activo) => {
    try {
      const res = await fetch(
        `${API_BASE}/auth/usuarios/${encodeURIComponent(username)}/activo`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json", ...authHeaders },
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <select
              ...
              value={newUser.rol}
              onChange={(e) =>
                setNewUser((p) => ({ ...p, rol: e.target.value }))
              }
          ...
          <button
            ...
            disabled={saving}
            style={{ ...btnPrimary, alignSelf: "end" }}
          >
            {saving ? (
              <RefreshCw
                size={13}
            ...
            <tbody>
              {users.map((u, i) => (
                ...
                >
                  ...
                  >
                    {u.ultimo_acceso
                      ? u.ultimo_acceso.slice(0, 16).replace("T", " ")
                      : "Nunca"}
                  </td>
                  ...
                  <td style={{ padding: "8px 12px" }}>
                    {u.username !== usuario?.username && (
                      <div style={{ display: "flex", gap: 6 }}>
                        <button
                          onClick={() =>
                            handleToggleActivo(u.username, !u.activo)
                          }
...
function KillSwitchPanel({ token }) {
  ...
  const [loading, setLoading] = useState(false);
  const [msg, setMsg] = useState(null);
  const [licTexto, setLicTexto] = useState("");
  ...
  const toggle = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.detail ?? "Error al cambiar kill switch.");
      setMsg({
        type: "ok",
        text: `Sistema ${nuevoEstado ? "activado" : "bloqueado"}.`,
      });
      setEstado(data);
    } catch (e) {
  ...
  const instalarLicencia = async () => {
    setSavingLic(true);
    setMsg(null);
    try {
      const res = await fetch(`${API_BASE}/kill-switch/licencia`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...authHeaders },
        body: JSON.stringify({ contenido: licTexto.trim() }),
      });
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.detail ?? "Error al instalar la licencia.");
      setMsg({ type: "ok", text: "Licencia instalada y verificada." });
      setLicTexto("");
      cargar();
    } catch (e) {
      setMsg({ type: "error", text: e.message });
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          >
            {activo
              ? "Sistema ACTIVO — Agentes pueden ejecutar"
              : "Sistema BLOQUEADO — Ejecución detenida"}
          </div>
          <div
            style={{ fontSize: 10, color: "var(--t-text-muted)", marginTop: 2 }}
          >
            Fuente: {estado?.fuente ?? "—"}
            {estado?.licencia_valida && (
              <span style={{ marginLeft: 8, color: "var(--t-accent)" }}>
                · Licencia {estado?.edicion ?? ""}{" "}
                {estado?.expira ? `(expira ${estado.expira})` : "(perpetua)"}
              </span>
            )}
            {estado?.licencia_presente && !estado?.licencia_valida && (
              <span style={{ marginLeft: 8, color: "#ef4444" }}>
                · Licencia inválida: {estado?.motivo}
              </span>
            )}
          </div>
        </div>
      ...
      <div>
        ...
        >
          <Key size={12} />
          {showLic ? "Ocultar" : "Instalar licencia (license.key)"}
        </button>
        ...
        {showLic && (
          ...
          >
            <label
              htmlFor="sec-licencia"
              style={{ fontSize: 11, color: "var(--t-text-muted)" }}
            >
              Pega el contenido completo de tu archivo license.key (JSON
              firmado)
            </label>
            <textarea
              id="sec-licencia"
              value={licTexto}
              onChange={(e) => setLicTexto(e.target.value)}
              placeholder={'{ "payload": { "machine_id": "…" }, "firma": "…" }'}
              rows={4}
              style={{
                ...input,
                fontFamily: "monospace",
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <button
                onClick={instalarLicencia}
                disabled={savingLic || !licTexto.trim()}
                style={btnPrimary}
              >
                {savingLic ? (
                  <RefreshCw
              ...
              <span style={{ fontSize: 10, color: "var(--t-text-muted)" }}>
                ID de esta máquina:{" "}
                <code style={{ fontSize: 9 }}>{estado?.machine_id ?? "—"}</code>
              </span>
            </div>
            <p
              style={{
            ...
            >
              La licencia se valida localmente (firma RSA + ID de hardware), sin
              conexión a internet. Sin licencia, el sistema funciona en modo
              desktop libre.
            </p>
          </div>
        )}
      ...
      <div
        style={{ fontSize: 11, color: "var(--t-text-muted)", lineHeight: 1.6 }}
      >
        El Kill Switch bloquea <strong>inmediatamente</strong> la ejecución de
        todos los agentes. Los agentes en curso terminan su filtro actual y se
        detienen en el siguiente.
      </div>
...
function ChangePasswordSection({ token }) {
  ...
  const handleChange = async () => {
    if (!username || !newPw || newPw.length < 8) {
      setMsg({
        type: "error",
        text: "Usuario y contraseña ≥8 caracteres requeridos.",
      });
      return;
    }
    setLoading(true);
...
function ManualDownload() {
  ...
  const descargar = async () => {
    setDescargando(true);
    try {
      const param = empresa.trim()
        ? `?empresa=${encodeURIComponent(empresa.trim())}`
        : "";
  ...
  return (
    ...
    >
      ...
      >
        <label
          htmlFor="sec-manual-empresa"
          style={{ fontSize: 11, color: "var(--t-text-muted)" }}
        >
          Nombre de empresa (opcional)
        </label>
        <input
          id="sec-manual-empresa"
          value={empresa}
    ...
    >
      ...
      <div style={card}>
        ...
        <p style={{ margin: 0, fontSize: 12, color: "var(--t-text-muted)" }}>
          El hash se calcula localmente en el navegador con bcryptjs. Úsalo para
          MASTER_PASSWORD_HASH o al crear un usuario sin exponer la contraseña
          en texto claro.
        </p>
        <HashGenerator />
      </div>
      ...
      <div style={card}>
        ...
        <p style={{ margin: 0, fontSize: 12, color: "var(--t-text-muted)" }}>
          ...
          Incluye guía de la Curva S, gestión de roles y configuración del Kill
          Switch.
        </p>
        <ManualDownload />
      </div>

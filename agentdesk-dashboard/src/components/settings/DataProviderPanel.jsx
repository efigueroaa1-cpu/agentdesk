...
function DropZone({ onFiles }) {
  ...
  return (
    ...
    >
      <Upload
        size={28}
        style={{ color: "var(--t-accent)", opacity: dragging ? 1 : 0.5 }}
      />
...
function FilePreview({ file, preview }) {
  ...
  return (
    ...
    >
      ...
      {preview && (
        <pre
          className="text-xs rounded-lg p-3 overflow-auto max-h-36 font-mono"
          style={{
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        setStatus("error");
        setErrMsg(data.error ?? "Conexión fallida.");
      }
    } catch {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {estado === "listo" && (
        ...
        >
          ✓ {archivos.length} archivo{archivos.length !== 1 ? "s" : ""} enviado
          {archivos.length !== 1 ? "s" : ""}.{" "}
          <strong>Ve a la pestaña Agentes → Chat</strong> para analizarlos con
          un agente.
        </div>
      )}
      {estado === "error" && (

...
export default function ReportsPanel() {
  ...
  const descargar = async (nombre) => {
    ...
    try {
      const r = await fetch(`${API_BASE}/reportes/${encodeURIComponent(nombre)}`, {
        headers: { Authorization: `Bearer ${TOKEN()}` },
      });
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ margin: "2px 0 0", fontSize: "0.78rem", color: "var(--t-text-muted)" }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <input
            ...
            style={{
              ...
              borderRadius: 8, color: "var(--t-text)", padding: "5px 11px",
              fontSize: "0.78rem", fontFamily: "inherit", outline: "none", width: 160,
            }}
          />
          {/* Filtros */}
          {[["todos","Todos"], ["pdf","Solo PDF"]].map(([k, l]) => (
            <button
              key={k}
              onClick={() => setFiltro(k)}
              style={{
                padding: "5px 13px", borderRadius: 20, fontSize: "0.72rem",
                fontWeight: 600, cursor: "pointer", fontFamily: "inherit",
                border: filtro === k ? "1.5px solid var(--t-accent)" : "1px solid var(--t-border)",
                background: filtro === k ? "rgba(0,212,255,.12)" : "transparent",
          ...
          <button
            ...
            style={{
              ...
              color: "var(--t-text-muted)", cursor: "pointer", fontFamily: "inherit",
            }}
          >
            ↺ Actualizar
          </button>
      ...
      {!loading && (
        <div style={{ fontSize: "0.76rem", color: "var(--t-text-muted)" }}>
          ...
          {reportes.length !== filtrados.length && ` de ${reportes.length} total`}
        </div>
      )}

      {/* Estado cargando */}
      {loading && (
        <div style={{ ...card, padding: "3rem", textAlign: "center", color: "var(--t-text-muted)" }}>
      ...
      {!loading && filtrados.length === 0 && (
        <div style={{ ...card, padding: "4rem 2rem", textAlign: "center" }}>
          <div style={{ fontSize: "3.5rem", opacity: 0.2, marginBottom: 12 }}>📄</div>
          <div style={{ color: "var(--t-text)", fontWeight: 600, marginBottom: 6 }}>
            No hay reportes aún
          </div>
          <div style={{ color: "var(--t-text-muted)", fontSize: "0.82rem", lineHeight: 1.6 }}>
            {busqueda ? (
              <>No se encontraron resultados para "{busqueda}".</>
            ) : (
              <>
                Conversa con cualquier agente en la pestaña <strong>Agentes → Chat</strong>
                <br />y usa el botón <strong>↓ PDF</strong> que aparece bajo cada respuesta.
              </>
            )}
          </div>
        </div>
      ...
      {!loading && filtrados.length > 0 && (
        <div style={{ ...card, overflow: "hidden" }}>
          ...
          <div style={{
            ...
            color: "var(--t-text-muted)", textTransform: "uppercase", letterSpacing: ".05em",
          }}>
          ...
          {filtrados.map((rep, i) => (
            <div
              ...
              style={{
                ...
                alignItems: "center",
                borderBottom: i < filtrados.length - 1 ? "1px solid var(--t-border)" : "none",
            ...
            >
              {/* Nombre */}
              <div>
                <div style={{ fontSize: "0.82rem", color: "var(--t-text)", fontWeight: 500 }}>
                  {tipoReporte(rep.nombre)} — {nombreAgente(rep.nombre)}
                </div>
                <div style={{ fontSize: "0.68rem", color: "var(--t-text-muted)", marginTop: 2, fontFamily: "monospace" }}>
                  {rep.nombre}
                </div>
              </div>
              ...
              {/* Tamaño */}
              <div style={{ fontSize: "0.78rem", color: "var(--t-text-muted)", textAlign: "right", whiteSpace: "nowrap" }}>
                {fmtTam(rep.tamano_kb)}
              </div>

              {/* Fecha */}
              <div style={{ fontSize: "0.73rem", color: "var(--t-text-muted)", textAlign: "right", whiteSpace: "nowrap" }}>
                {fmtFecha(rep.mtime)}
              </div>
              ...
              <div style={{ textAlign: "center" }}>
                <button
                  ...
                  style={{
                    ...
                    background:   descargando === rep.nombre
                      ? "rgba(0,212,255,0.05)"
                      : "rgba(0,212,255,0.10)",
                    color:        "var(--t-accent)",
                    cursor:       descargando === rep.nombre ? "not-allowed" : "pointer",
                    fontSize:     "0.75rem",
                    fontWeight:   600,
                    fontFamily:   "inherit",
                    whiteSpace:   "nowrap",
      ...
      {/* Nota informativa */}
      <div style={{
        fontSize: "0.73rem", color: "var(--t-text-muted)",
        padding: "8px 12px",
        ...
        <code style={{ fontSize: "0.70rem" }}>%APPDATA%\AgentDesk\reportes\</code>

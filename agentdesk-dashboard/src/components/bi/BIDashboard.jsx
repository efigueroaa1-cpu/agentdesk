...
export default function BIDashboard() {
  ...
  const guardrailData = Object.entries(compliance.por_guardrail ?? {})
  ...
  const alertasCompliance = (compliance.alertas_nivel ?? [])
    .map((a) => ({
      ...
      mensaje: `${a.total_abortos} abortos — ${Object.entries(
        a.guardrails ?? {},
      )
        .map(([k, v]) => `${k.replace("Guard", "")}: ${v}`)
        .join(", ")}`,
    }))
    .slice(0, 4);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", flex: 1 }}>
          ...
          <KPICard
            label="Avance Global"
            value={
              gantt.avance_promedio != null
                ? `${fmt(gantt.avance_promedio)}%`
                : "—"
            }
          ...
          <KPICard
            label="Certificado"
            value={
              compliance.certificado === true
                ? "✓ OK"
                : compliance.certificado === false
                  ? "✗ Riesgo"
                  : "—"
            }
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <Section title="Avance de Proyectos Gantt">
          {proyectos.length === 0 ? (
            ...
            proyectos
              .slice(0, 8)
              .map((p) => <ProyectoBar key={p.proyecto_id} p={p} />)
          )}
        </Section>
        ...
        <Section title="Abortos por Guardrail (últimos 30 días)">
          {guardrailData.length === 0 ? (
            <div style={{ color: C.muted, fontSize: 12 }}>
              Sin eventos de guardrail registrados.
            </div>
          ) : (
      ...
      <Section
        ...
        accent={
          alertasTotales.some((a) => a.nivel === "CRITICO")
            ? C.critico
            : C.border
        }
      >
      ...
      {data?.generado_en && (
        <div style={{ textAlign: "right", color: C.muted, fontSize: 10 }}>
          Datos de servidor: {data.generado_en.replace("T", " ").slice(0, 19)}{" "}
          UTC
        </div>
      )}
    </div>

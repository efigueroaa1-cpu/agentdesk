...
function HistoricoFinanciero({ agenteId }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(
        `${API_BASE}/finanzas/historico/${encodeURIComponent(agenteId)}?n=12`,
      ).then((r) => (r.ok ? r.json() : [])),
      fetch(
        `${API_BASE}/finanzas/tendencia/${encodeURIComponent(agenteId)}`,
  ...
  if (loading)
    return (
      <div style={{ color: C.muted, fontSize: 12 }}>Cargando histórico…</div>
    );
  if (!agenteId)
    return (
      <div style={{ color: C.muted, fontSize: 12 }}>Selecciona un agente.</div>
    );
  if (hist.length === 0)
    return (
      <div style={{ color: C.muted, fontSize: 12 }}>
        Sin análisis financieros guardados para este agente.
      </div>
    );
...
function ComplianceTrend() {
  ...
  if (loading)
    return (
      <div style={{ color: C.muted, fontSize: 12 }}>Cargando compliance…</div>
    );
  if (!reporte)
    return (
      <div style={{ color: C.muted, fontSize: 12 }}>
  ...
  const agentData = Object.entries(reporte.por_agente ?? {})
    .map(([k, v]) => ({
      name: k,
      value: Object.values(v).reduce((a, b) => a + b, 0),
    }))
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <span
            style={{
              color: reporte.certificado ? C.ok : C.critico,
              fontSize: 12,
              fontWeight: 700,
            }}
          >
  ...
  useEffect(() => {
    fetch(`${API_BASE}/agentes`)
      ...
      .then((data) => {
        ...
        setAgentes(lista);
        if (lista.length > 0 && !agenteId)
          setAgenteId(lista[0].id ?? lista[0].agente_id ?? "");
      })
      .catch(() => {});
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <label
          htmlFor="tendencias-agente"
          style={{ color: C.muted, fontSize: 11 }}
        >
          Agente:
        </label>
        <select
          id="tendencias-agente"
          value={agenteId}

...
class TestDispatcherCobertura(unittest.TestCase):
    ...
    def test_01_toda_tool_del_schema_es_despachable(self):
        """Cada tool del schema se resuelve sin NameError ni 'no reconocida'.
        Args vacios: las tools degradan a un error controlado (su try/except),
        lo que NO nos importa aqui -- solo que el cableado exista."""
        fallos = []
        for nombre in self._nombres_schema():
            r = _run(ejecutar_herramienta(nombre, {}))
            if "no reconocida" in r or "is not defined" in r:
                fallos.append(f"{nombre}: {r[:80]}")
        self.assertEqual(fallos, [], f"tools mal cableadas: {fallos}")
